...
function runValidation(): EvalReport {
  ...
  if (!composedBootstrapCase) {
    throw new Error("Missing case generated-reviewed-bootstrap-compose-ready");
  }
  try {
    ...
    results.push(
      result(
        ...
        composedBootstrapCase.failure_mode,
        "Generated init bootstrap targets composed in one isolated repo without broad scaffold writes.",
      ),
    );
  } catch (error: unknown) {
    results.push(
      result(
        composedBootstrapCase.id,
        false,
        ["generated reviewed bootstrap composition"],
        composedBootstrapCase.failure_mode,
        error instanceof Error ? error.message : "unknown generated reviewed bootstrap composition error",
      ),
    );
  }
