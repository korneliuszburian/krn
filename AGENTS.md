...
3. If a `/goal` is active, read that execution contract first. The active parent goal is [docs/goals/goal-006.md](/home/krn/coding/krn/active/krn-gastown/docs/goals/goal-006.md). The latest completed Slice 3 child goal is [docs/goals/goal-033.md](/home/krn/coding/krn/active/krn-gastown/docs/goals/goal-033.md); [docs/goals/goal-032.md](/home/krn/coding/krn/active/krn-gastown/docs/goals/goal-032.md) is the immediately previous expanded-arena fixture-scoring gate. Load older child goals only when the active goal, changed files, compact selector, or source ledger points to them.
...
Every non-trivial KRN slice should move through this pipeline:

```text
research/source pattern
  -> active goal with overclaim boundary
  -> typed object or registry
  -> deterministic eval with known-bad fixture
  -> runtime artifact under .krn/
  -> aggregate krn eval coverage
  -> canonical source/memory update
