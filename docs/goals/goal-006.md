...
Next child-goal candidates after `goal-030` are implementing the expanded benchmark/autoresearch arena from `docs/evals/krn-benchmark-arena-contract/arena-contract.example.json`, read-only repair-record MCP/dashboard surfacing, HTTP/API read model hardening, or Skill Impact / Goal Continuity surfaces. The next benchmark slice should implement the expanded suite contract first: at least 20 tasks, explicit live mode, progress/resume/smoke/full lanes, separate fixture/live evidence, coding-quality rubric dimensions, and implementation-heavy task families. Do not expand dashboard command surfaces before the arena implementation exists. Do not add explicit live benchmark runner mode to default deterministic `krn eval`, expose destructive MCP/API tools, mocked dashboard state, broad promotion mutation, dashboard rerun/repair commands, or productivity claims from three-task live benchmark evidence, a proposed repair record, a green live shape report, a stability classifier pass, repeated clean small-suite evidence, a green arena-contract eval, or positive suite deltas below the lift gate.
