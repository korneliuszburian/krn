...
Next child-goal candidates after `goal-032` are implementing isolated explicit live smoke/full runner modes for `docs/evals/krn-benchmark-expanded-arena/tasks.json`, read-only repair-record MCP/dashboard surfacing, HTTP/API read model hardening, or Skill Impact / Goal Continuity surfaces. The next benchmark slice should keep the expanded registry and fixture scorer as the source of truth, add isolated live runner evidence, and keep live `codex exec` outside default deterministic `krn eval`. Do not expand dashboard command surfaces before live runner evidence exists. Do not add explicit live benchmark runner mode to default deterministic `krn eval`, expose destructive MCP/API tools, mocked dashboard state, broad promotion mutation, dashboard rerun/repair commands, or productivity claims from three-task live benchmark evidence, a proposed repair record, a green live shape report, a stability classifier pass, repeated clean small-suite evidence, a green arena-contract eval, a green expanded-registry eval, a green fixture-scoring eval, or positive fixture deltas below the live lift gate.
