...
Next child-goal candidates after `goal-033` are repairing expanded-arena worker prompt/budget ergonomics until live-smoke completes a selected task cleanly, then running or hardening `live-full`, read-only repair-record MCP/dashboard surfacing, HTTP/API read model hardening, or Skill Impact / Goal Continuity surfaces. The next benchmark slice should keep the expanded registry, fixture scorer, and explicit live runner contract as the source of truth, and should not treat the current timeout smoke as useful productivity evidence. Do not expand dashboard command surfaces before clean live runner evidence exists. Do not add explicit live benchmark runner mode to default deterministic `krn eval`, expose destructive MCP/API tools, mocked dashboard state, broad promotion mutation, dashboard rerun/repair commands, or productivity claims from three-task live benchmark evidence, a proposed repair record, a green live shape report, a stability classifier pass, repeated clean small-suite evidence, a green arena-contract eval, a green expanded-registry eval, a green fixture-scoring eval, positive fixture deltas, or a timeouting expanded-arena live-smoke run below the live lift gate.
