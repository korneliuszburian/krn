...
| LOCAL014 | `packages/mcp/src/server.ts`, `packages/mcp/src/stdio.ts`, `packages/mcp/test/stdio-server.test.ts`, `packages/evals/src/validate-krn-mcp-transport.ts`, and `docs/evals/krn-mcp-transport/` implement the first local STDIO MCP transport; `.krn/evals/krn-mcp-transport/20260619T224349Z-1769625/report.json` shows 3/3 cases and 7/7 assertions passing, and `.krn/eval/20260619T224349Z-1769637/report.json` aggregates 5/5 modules, 15/15 cases, and 37/37 assertions. | Slice 3 now has a real local STDIO MCP resource transport, but it is not registered in Codex config, not an HTTP/ChatGPT connector, and not a write/proposal tool surface. |
...
| C026 | The first local `packages/mcp` STDIO transport proves allowlisted read-only resources can be served through the MCP SDK boundary without tools, not Codex config registration, HTTP/ChatGPT connector behavior, proposal/write-tool safety, dashboard readiness, or productivity lift. | S007, S022, C004, C025, LOCAL014 | A/local | yes | A green local STDIO eval is overclaimed as deployed integration or permission to expose write tools. |
