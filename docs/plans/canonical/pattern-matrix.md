...
| Codex Bootstrap | Proposal-only init target before apply mode | C029, C032, C067-C068, LOCAL053 | local | Routes `agent_instructions` into append-only proposal review backed by the generated manifest before any `AGENTS.md` mutation | Strong fit | Init jumps from dry-run to direct target writes or hides bootstrap truth in CLI branches | adopt |
| Codex Bootstrap | Reviewed init local-config target | C029, C032, C067-C070, LOCAL055 | local | Writes `.krn/config.toml` only through approved exact payload promotion and keeps memory core external via `KRN_MEMORY_STORE_PATH` | Strong fit | Config becomes repo-local memory core, copied active-goal truth, or a parallel direct writer | adopt |
| Codex Bootstrap | Reviewed init source-pointers target | C029, C032, C067-C071, LOCAL056 | local | Writes `.krn/sources/index.json` only through approved exact payload promotion and seeds a source graph boundary without copying canonical source truth | Strong fit | Source graph seed becomes a copied bibliography, freshness claim, or parallel direct writer | adopt |
| Codex Bootstrap | Reviewed init context-pointers target | C029, C032, C067-C072, LOCAL057 | local | Writes `.krn/context/index.json` only through approved exact payload promotion and seeds a context packet pointer boundary without memory bodies, active task truth, or broad docs dumps | Strong fit | Context pointer seed becomes a hidden memory store, copied active goal/canonical plan, or parallel direct writer | adopt |
| Codex Bootstrap | Reviewed init eval-baseline target | C029, C032, C066-C073, LOCAL058 | local | Writes `.krn/evals/baseline.json` only through approved exact payload promotion and seeds a core/current eval lane policy without lab/all defaults, live report IDs, or lift claims | Strong fit | Eval baseline seed becomes a copied live report, benchmark proof, all-lane default, or parallel direct writer | adopt |
