...
[DECISION] The first final-product bottlenecks after reset are the `MemoryStore` boundary, pre-edit engineering gate, bounded context packet, local source graph check, eval-lane split, final-shaped `krn init --dry-run` bootstrap, and the reviewed `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, and `eval_baseline` proposal/apply paths. They keep memory outside repo-file truth, select IDs with reasons/confidence/source lineage, apply guidance to a real CLI/review/context consumer, record runtime IDs and feedback, fail context dumps, reject selected memory without application guidance, block stale/conflicting selected source refs, keep lab/dashboard/benchmark history out of default verification, expose bootstrap capabilities without direct dry-run mutation, write the first absent `AGENTS.md` only through approved review plus exact payload promotion, write the minimal `.krn/config.toml` only as an exact reviewed local-config payload that points to an external local MemoryStore, write the minimal `.krn/sources/index.json` only as an exact reviewed source graph seed that does not copy active source truth, write the minimal `.krn/context/index.json` only as an exact reviewed context pointer seed that does not copy memory bodies, active task truth, `goal-038`, or the canonical draft, and write the minimal `.krn/evals/baseline.json` only as an exact reviewed eval-baseline payload that keeps lab/all defaults and productivity-lift claims out of bootstrap. The next useful bottleneck is skill/policy bootstrap readiness or repo-bootstrap readiness, not another passive planning artifact, dashboard, benchmark, broad API/cloud sync, or research runtime.
...
| Completed foundation | MemoryStore boundary, pre-edit gate, bounded context packet, source graph check, eval-lane split, final-shaped `krn init --dry-run` bootstrap, reviewed `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, and `eval_baseline` proposal/apply paths | Store-backed selection/application/feedback, `krn gate`, `krn context build`, `krn sources check`, lane-aware `krn eval`, required init bootstrap capabilities, runtime IDs, rejected context dumps, selected-memory application guidance, source pass/warn/block decisions, lab exclusion by default, dry-run mutation rejection, exact absent-`AGENTS.md` write through approved promotion, exact absent-`.krn/config.toml` write through approved promotion with memory core kept behind `KRN_MEMORY_STORE_PATH`, exact absent-`.krn/sources/index.json` write through approved source graph seed promotion, exact absent-`.krn/context/index.json` write through approved context pointer seed promotion, and exact absent-`.krn/evals/baseline.json` write through approved eval-baseline promotion that keeps lab/all/lift claims out of bootstrap | The proof is overclaimed as final memory/source/eval/context quality, broad repo bootstrap, merge-mode safety, research automation, broad config consumption, eval quality, or productivity lift |
| Next | Skill wiring, policy boundaries, or repo-bootstrap readiness | KRN can move from reviewed agent/config/source/context/eval target setup toward usable repo bootstrap without broad scaffolding, hardcoded truth, or repo-local memory-core fiction | Bootstrap writes broad scaffolding, hardcodes product truth, skips review/promotion, overwrites existing instructions, treats `.krn` as memory core, or creates more overhead than it removes |
...
- What is the next narrow `krn init` capability after exact `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, and `eval_baseline` apply: skill wiring, policy boundaries, or repo-bootstrap readiness?
