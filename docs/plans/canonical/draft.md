...
[DECISION] The execution source of truth is now this canonical blueprint plus [docs/goals/goal-038.md](../../goals/goal-038.md). [docs/product/final-product-plan.md](../../product/final-product-plan.md) is a compatibility pointer, while `goal-006` and `goal-037` are historical evidence. `goal-005` is no longer the active product direction; it is Slice 2 context for `krn init --dry-run`.

[DECISION] [docs/goals/goal-038.md](../../goals/goal-038.md) absorbs the `goal-037` engineering-kernel reset into one final-product endgame. Non-trivial work must remain mechanism-first, bottleneck-led, production-shaped, context-budgeted, diff-literate, review-minimizing, memory-operative, proof-carrying, and deletion-friendly.

[DECISION] The first final-product bottlenecks after reset are the `MemoryStore` boundary, pre-edit engineering gate, bounded context packet, local source graph check, eval-lane split, final-shaped `krn init --dry-run` bootstrap, the reviewed `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, `eval_baseline`, `skill_wiring`, and `policy_boundaries` proposal/apply paths, and the isolated reviewed-bootstrap composition proof. They keep memory outside repo-file truth, select IDs with reasons/confidence/source lineage, apply guidance to a real CLI/review/context consumer, record runtime IDs and feedback, fail context dumps, reject selected memory without application guidance, block stale/conflicting selected source refs, keep lab/dashboard/benchmark history out of default verification, expose bootstrap capabilities without direct dry-run mutation, write the first absent `AGENTS.md` only through approved review plus exact payload promotion, write the minimal `.krn/config.toml` only as an exact reviewed local-config payload that points to an external local MemoryStore, write the minimal `.krn/sources/index.json` only as an exact reviewed source graph seed that does not copy active source truth, write the minimal `.krn/context/index.json` only as an exact reviewed context pointer seed that does not copy memory bodies, active task truth, `goal-038`, or the canonical draft, write the minimal `.krn/evals/baseline.json` only as an exact reviewed eval-baseline payload that keeps lab/all defaults and productivity-lift claims out of bootstrap, write the minimal `.agents/skills/README.md` only as an exact reviewed skill-wiring seed that does not copy active skill bodies, active-goal truth, memory bodies, or skill-quality claims, write the minimal `.krn/policies/boundaries.json` only as an exact reviewed policy-boundary payload that blocks repo-local memory-core writes, requires target-write approval, handles source acceptance, warns on command execution, blocks dashboard/API expansion, and forbids cloud/dashboard/benchmark/lift defaults, prove all reviewed targets compose in one isolated target without repo-local memory-core writes or hardcoded product truth, keep `krn gate` strict enough to block broad forbidden-surface expansion while allowing explicitly negated hardcoded-truth boundaries such as `no dashboard state`, and keep init target metadata in one registry without moving exact payload generation into a generic scaffold engine. The next useful bottleneck is the capability dogfood loop: KRN must use its own context/source/memory/eval/review boundaries on one real repo capability task, not add another passive planning artifact, init proof, dashboard, benchmark, broad API/cloud sync, research runtime, or registry cleanup without a new consumer.

[DECISION] This blueprint describes the full target state. Implementation slices are dependency-ordered carriers of the final architecture, not MVP, v0, prototype, or demo stages. The active goal must keep the final ADR/source-backed pattern catalog current while code moves as fast as the evidence and rollback boundaries allow.

[DECISION] The active cleanup rule is now part of the product path: before the next surface, KRN must reduce repo sediment in the touched path by deleting, parking, merging, or routing stale goals, duplicate ledgers, hardcoded local paths, unconsumed eval/lab surfaces, and monolith pressure out of the default workflow.

[DECISION] KRN-to-build-KRN is future-gated dogfood, not an assumption. A KRN layer may help build KRN only when it already has a typed boundary, consumer, verification, rollback/kill path, and overclaim boundary.
...
| Completed foundation | MemoryStore boundary, pre-edit gate, bounded context packet, source graph check, eval-lane split, final-shaped `krn init --dry-run` bootstrap, reviewed `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, `eval_baseline`, `skill_wiring`, and `policy_boundaries` proposal/apply paths, and isolated repo-bootstrap readiness composition | Store-backed selection/application/feedback, `krn gate`, `krn context build`, `krn sources check`, lane-aware `krn eval`, required init bootstrap capabilities, runtime IDs, rejected context dumps, selected-memory application guidance, source pass/warn/block decisions, lab exclusion by default, dry-run mutation rejection, exact absent-`AGENTS.md` write through approved promotion, exact absent-`.krn/config.toml` write through approved promotion with memory core kept behind `KRN_MEMORY_STORE_PATH`, exact absent-`.krn/sources/index.json` write through approved source graph seed promotion, exact absent-`.krn/context/index.json` write through approved context pointer seed promotion, exact absent-`.krn/evals/baseline.json` write through approved eval-baseline promotion that keeps lab/all/lift claims out of bootstrap, exact absent-`.agents/skills/README.md` write through approved skill-wiring seed promotion that avoids copied skill bodies and active-goal truth, exact absent-`.krn/policies/boundaries.json` write through approved policy-boundary promotion that blocks repo-local memory core, cloud/default dashboard/API drift, and hook/security/lift overclaims, a `krn gate` false-positive repair for negated forbidden-surface boundaries, one stable init target metadata registry, and an isolated full reviewed-bootstrap sequence whose second dry-run marks reviewed targets as `skip` without creating `docs/memory/**`, `.krn/memory/**`, dashboard/API state, copied `goal-038`, or copied canonical draft truth | The proof is overclaimed as final memory/source/eval/context quality, broad repo bootstrap, merge-mode safety, research automation, broad config consumption, eval quality, policy hook enforcement, security quality, semantic policy enforcement, skill quality, fresh-repo dogfood success, or productivity lift |
| Next | Capability dogfood loop | KRN can take one real repo capability task, build bounded context from existing MemoryStore/source boundaries, run through reviewed bootstrap/task evidence, verify with focused tests/evals, and record keep/kill/revisit without adding another passive init, memory, dashboard, benchmark, or API layer | KRN keeps proving infrastructure in isolation, adds task/memory/dashboard surfaces before they are consumed by a real repo task, or claims productivity lift before review burden/diff risk improves |
...
- What is the tightest eval registry split that keeps core/current gates fast while parking lab/dashboard/benchmark history outside the default path?
...
- What is the tightest dashboard object model worth prototyping?
- What is the tightest capability dogfood proof where KRN uses its own context/source/memory/eval/review boundaries to improve one real repo task before dashboard/API/cloud expansion?
...
| 2026-06-21 | Fifth reviewed `krn init` apply target joins the active execution path. | `krn init --proposal/apply eval_baseline` writes an absent `.krn/evals/baseline.json` only after an approved review decision and exact `init_eval_baseline` payload promotion, while keeping live eval report IDs, lab/all defaults, productivity-lift claims, active goal/canonical plan text, dashboard state, API sync, and memory-core truth out of the seed. |
| 2026-06-21 | Sixth reviewed `krn init` apply target joins the active execution path. | `krn init --proposal/apply policy_boundaries` writes an absent `.krn/policies/boundaries.json` only after an approved review decision and exact `init_policy_boundaries` payload promotion, while blocking repo-local memory-core writes, requiring target-write approval, handling source acceptance, warning on command execution, blocking dashboard/API expansion, and keeping hook/security/productivity overclaims out of the seed. |
| 2026-06-21 | `krn gate` negated-boundary repair joins the active execution path. | The pre-edit gate now blocks broad dashboard/benchmark/API/cloud/live-full expansion without false-blocking scoped tasks that name those same surfaces in `no ...` hardcoded-truth or overclaim boundaries. |
| 2026-06-21 | Init target registry simplification joins the active execution path. | `krn init` now has one target metadata registry for parser capability lists, usage help, manifest planning, and proposal metadata, while exact payload builders remain explicit in the reviewed promotion path. |
| 2026-06-21 | Seventh reviewed `krn init` apply target joins the active execution path. | `krn init --proposal/apply skill_wiring` writes an absent `.agents/skills/README.md` only after an approved review decision and exact `init_skill_wiring` payload promotion, while keeping copied skill bodies, memory bodies, active-goal truth, hook/security claims, and productivity-lift claims out of the seed. |
| 2026-06-21 | Reviewed bootstrap composition closes the isolated init-proof sequence. | One isolated target now proves dry-run, seven proposals, approved decisions, exact applies, and post-apply dry-run `skip` detection without repo-local memory core, dashboard/API state, copied `goal-038`, or copied canonical draft truth. |
| 2026-06-21 | Cleanup and capability dogfood become the active execution direction. | The next KRN work must reduce repo sediment in the touched path and then use existing KRN boundaries on one real capability task before adding more isolated proof surfaces. KRN-to-build-KRN remains future-gated until the involved layer has typed boundary, consumer, verification, rollback, and overclaim evidence. |
...
- Local evidence entries: LOCAL001-LOCAL064.
- Active reset claims: C059-C078.
