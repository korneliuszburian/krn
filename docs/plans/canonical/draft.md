...
- What is the next narrow `krn init` capability after exact `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, `eval_baseline`, and `policy_boundaries` apply: skill wiring, repo-bootstrap readiness, or a small target-registry simplification?
- Does `packages/cli/src/init-bootstrap.ts` need a stronger target registry before skill wiring/repo-bootstrap readiness, or is the current payload boundary enough?
...
- Local evidence entries: LOCAL001-LOCAL060.
- Active reset claims: C059-C075.
