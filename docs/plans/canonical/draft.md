...
[DECISION] The first final-product bottlenecks after reset are the `MemoryStore` boundary, pre-edit engineering gate, bounded context packet, local source graph check, eval-lane split, final-shaped `krn init --dry-run` bootstrap, and the reviewed `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, `eval_baseline`, and `policy_boundaries` proposal/apply paths. They keep memory outside repo-file truth, select IDs with reasons/confidence/source lineage, apply guidance to a real CLI/review/context consumer, record runtime IDs and feedback, fail context dumps, reject selected memory without application guidance, block stale/conflicting selected source refs, keep lab/dashboard/benchmark history out of default verification, expose bootstrap capabilities without direct dry-run mutation, write the first absent `AGENTS.md` only through approved review plus exact payload promotion, write the minimal `.krn/config.toml` only as an exact reviewed local-config payload that points to an external local MemoryStore, write the minimal `.krn/sources/index.json` only as an exact reviewed source graph seed that does not copy active source truth, write the minimal `.krn/context/index.json` only as an exact reviewed context pointer seed that does not copy memory bodies, active task truth, `goal-038`, or the canonical draft, write the minimal `.krn/evals/baseline.json` only as an exact reviewed eval-baseline payload that keeps lab/all defaults and productivity-lift claims out of bootstrap, write the minimal `.krn/policies/boundaries.json` only as an exact reviewed policy-boundary payload that blocks repo-local memory-core writes, requires target-write approval, handles source acceptance, warns on command execution, blocks dashboard/API expansion, and forbids cloud/dashboard/benchmark/lift defaults, keep `krn gate` strict enough to block broad forbidden-surface expansion while allowing explicitly negated hardcoded-truth boundaries such as `no dashboard state`, and keep init target metadata in one registry without moving exact payload generation into a generic scaffold engine. The next useful bottleneck is skill wiring or repo-bootstrap readiness, not another passive planning artifact, dashboard, benchmark, broad API/cloud sync, research runtime, or registry cleanup without a new consumer.
...
| Completed foundation | MemoryStore boundary, pre-edit gate, bounded context packet, source graph check, eval-lane split, final-shaped `krn init --dry-run` bootstrap, reviewed `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, `eval_baseline`, and `policy_boundaries` proposal/apply paths | Store-backed selection/application/feedback, `krn gate`, `krn context build`, `krn sources check`, lane-aware `krn eval`, required init bootstrap capabilities, runtime IDs, rejected context dumps, selected-memory application guidance, source pass/warn/block decisions, lab exclusion by default, dry-run mutation rejection, exact absent-`AGENTS.md` write through approved promotion, exact absent-`.krn/config.toml` write through approved promotion with memory core kept behind `KRN_MEMORY_STORE_PATH`, exact absent-`.krn/sources/index.json` write through approved source graph seed promotion, exact absent-`.krn/context/index.json` write through approved context pointer seed promotion, exact absent-`.krn/evals/baseline.json` write through approved eval-baseline promotion that keeps lab/all/lift claims out of bootstrap, exact absent-`.krn/policies/boundaries.json` write through approved policy-boundary promotion that blocks repo-local memory core, cloud/default dashboard/API drift, and hook/security/lift overclaims, a `krn gate` false-positive repair for negated forbidden-surface boundaries, and one stable init target metadata registry | The proof is overclaimed as final memory/source/eval/context quality, broad repo bootstrap, merge-mode safety, research automation, broad config consumption, eval quality, policy hook enforcement, security quality, semantic policy enforcement, skill quality, repo-bootstrap readiness, or productivity lift |
| Next | Skill wiring or repo-bootstrap readiness | KRN can move from reviewed agent/config/source/context/eval/policy target setup toward usable repo bootstrap without broad scaffolding, hardcoded truth, repo-local memory-core fiction, or duplicated target mechanics | Bootstrap writes broad scaffolding, hardcodes product truth, skips review/promotion, overwrites existing instructions, treats `.krn` as memory core, creates more overhead than it removes, or `skill_wiring` becomes prompt-sprawl without exact payload/review/eval boundaries |
...
- What is the next narrow `krn init` capability after exact `agent_instructions`, `local_config`, `source_pointers`, `context_pointers`, `eval_baseline`, and `policy_boundaries` apply plus target-registry simplification: skill wiring or repo-bootstrap readiness?
- What exact payload boundary is safe enough for `skill_wiring` without turning skills into copied prompt sprawl?
