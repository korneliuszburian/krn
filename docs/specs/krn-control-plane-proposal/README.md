...
- optional machine-applicable `promotion_payload` for exact memory-entry, init agent-instructions, init local-config, or init source-pointers promotion,
- idempotency key,
- source refs,
- evidence refs,
- human review gate,
...
- carry first-step repo-bootstrap review input for `krn init --proposal agent_instructions|local_config|source_pointers` through `proposal_kind: "init_bootstrap"`,
