...
- [krn-benchmark-arena-contract](./krn-benchmark-arena-contract/README.md) checks whether the next expanded benchmark arena has a 20-task lift gate, coding-quality rubric, explicit live boundary, and resumable pipeline contract before implementation.
- [krn-benchmark-expanded-arena](./krn-benchmark-expanded-arena/README.md) checks whether the expanded benchmark arena has a 20-task task registry, deterministic fixture scoring, coding-quality metrics, explicit live smoke/full runner modes outside default eval, isolated worker evidence, bounded smoke-repair guard, and a no-lift caveat before product claims.
...
- [krn-init-contracts](./krn-init-contracts/README.md) checks whether `krn init --dry-run` emits a schema-backed manifest, whether `krn init --proposal agent_instructions|local_config|source_pointers|context_pointers|eval_baseline|skill_wiring|policy_boundaries` stores source-backed append-only `init_bootstrap` proposals without mutating targets, whether reviewed apply writes exact `AGENTS.md` / `.krn/config.toml` / `.krn/sources/index.json` / `.krn/context/index.json` / `.krn/evals/baseline.json` / `.agents/skills/README.md` / `.krn/policies/boundaries.json` payloads, and whether known-bad write/missing-capability fixtures fail.
