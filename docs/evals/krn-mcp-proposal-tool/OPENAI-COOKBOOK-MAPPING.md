...
|---|---|---|---|
| `proposal-tool-listed-with-read-resources` | Goals and ExecPlans require explicit boundaries and verification surfaces; MCP tools need allowlisted exposure. | Server exposes the five read-only resources plus exactly one proposal-only tool. | Tool surface grows silently or breaks read resources. |
